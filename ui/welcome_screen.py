...
def setup_welcome_screen(root, main_frame):  
    ...
    CTkButton(
        welcome_frame, text="New Experiment", 
        font=("Georgia", 80), 
        command=lambda: create_file(root, experiments_frame), 
        width=width, height=height
        ).pack(pady=(10, 5), padx=20, fill='x', expand=True)
    CTkButton(
        welcome_frame, text="Open Experiment", 
        font=("Georgia", 80), 
        command=lambda: open_file(root, experiments_frame), width=width, height=height
        ).pack(pady=(5, 10), padx=20, fill='x', expand=True)
    CTkButton(
        welcome_frame, text="Test Serials", 
        font=("Georgia", 80), 
        command=lambda: open_test(root), width=width, height=height
        ).pack(pady=(5, 10), padx=20, fill='x', expand=True)
