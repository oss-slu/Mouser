...
class ReviewUI(MouserPage):
    ...
    def __init__(self, parent: CTk, prev_page: CTkFrame, database_name: str = ""):
        ...
        name_label = CTkLabel(
            self.main_frame,
            text=f"Experiment name: {self.database.get_name()} /n Animals: {self.database.get_measurement_items()}",
        )
        ...
        additional_text_label.place(relx=0.5, rely=0.2, anchor=CENTER)
