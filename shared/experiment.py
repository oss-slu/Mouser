...
class Experiment():
    ...
    def save_to_database(self, directory: str):
        ...
        if isinstance(self.data_collect_type, tuple):
            measurement_type_str = ','.join(str(t) for t in self.data_collect_type) # pylint: disable=W0612
        else:
            measurement_type_str = str(self.data_collect_type) # pylint: disable=W0612
